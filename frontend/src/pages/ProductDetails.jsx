  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setProduct(found);
        const relatedProducts = await getRelatedProducts(found.id, found.category?.slug);
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12">
          <div className="relative flex h-[420px] items-center justify-center overflow-visible rounded-2xl glass-panel lg:col-span-7 lg:h-[600px]">
            ...
            {images.length ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-text-muted">
                <span className="material-symbols text-6xl">image</span>
              </div>
            )}
            {images.length > 1 ? (
          ...
          <div className="flex flex-col justify-center lg:col-span-5">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <Badge variant={inStock ? "lime" : "neutral"}>
                <span className="material-symbols text-[16px]">check_circle</span>
                {inStock ? "In Stock" : "Out of Stock"}
              </Badge>
              <Badge variant="neutral">{product.category?.name || product.category?.slug || "Product"}</Badge>
            </div>
            ...
            {product.seller?.store_name ? (
              <div className="rounded-xl border border-outline-variant/20 bg-surface/70 p-4">
            ...
            <div className="flex flex-col items-start gap-8 md:flex-row md:items-center">
              ...
              <div className="flex-grow">
                ...
                >
                  ...
                  <button
                    type="submit"
                    className="absolute right-2 top-1/2 flex h-10 w-10 -translate-x-1/2 items-center justify-center text-accent transition-colors hover:bg-surface-high"
